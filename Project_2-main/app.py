...
def home():
    ...
    <br>
    <a href="/api/v1.0/state/<state>">/api/v1.0/State Name and year</a>
    </li>
    <br>
...
@app.route("/test")
def test():
    return render_template("AirPollutionWebsite (1).html")
...
def getstate(state=None):
    session = Session(engine)
    get_state_name =state
    
    aqi_state_name_query = session.query(overall_aqi.id,overall_aqi.state_name,overall_aqi.county_name,overall_aqi.years,overall_aqi.aqi).filter(overall_aqi.state_name == get_state_name).all()
    # Convert list of tuples into normal list
